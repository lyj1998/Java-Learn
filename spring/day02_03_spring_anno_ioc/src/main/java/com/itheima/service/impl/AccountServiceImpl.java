 ...
 *   <property name="" value ="" |  ref=""></property>
 ...
 用于注入数据的：
            他们的作用就和在xml配置文件中的bean标签中写一个<property></property>标签的作用是一样的
             ...
             Qulifier:
                  在按照类中注入的基础之上再按照名称注入，不能单独使用，但是在给方法参数注入时可以
               属性：value: 用于注入beanid
             Resource
                   作用:直接按照bean的id注入。它可以独立使用
              属性：name:用于指定bean的id
             以上三个注入都只能呢个注入其他bean类型的数据，而基本类型和String类型无法使用上述注解实现。
             另外，集合类型的注入只能通过XML来实现。
            Value
                作用：用于注入基本类型和String类型的数据
                属性：
                   value:用于指定数据的值。它可以使用spring中SpEL(也就是pring的el表达式)
                         SpEl的写法：${表达式}
 用于改变作用范围的
             他们的作用就和再bean标签中使用scope属性实现的功能是一样的
             Scope:
                   属性：
                          value:指定范围的取值。常用取值：singleton prototype
 和生命周期相关
              他们的作用就和在bean中使用init-method和destroy-method的作用是一样的
              PreDestroy
                      作用：用于指定销毁方法
              PostConstruct:
                      作用：用于指定初始化方法
